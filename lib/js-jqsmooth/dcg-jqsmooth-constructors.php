...
function dfcg_jq_multioption_method_gallery() {
	...
	foreach ($query_pairs as $query_pair) {
		...
		if( !is_array($query_pair) ) {
			...
			$output .= "\n\t" . '</ul><!-- end #dfcg-slideshow -->';
			$output .= "\n" . '</div><!-- end #dfcg-outer-wrapper -->';
			$output .= "\n\n" . '<!-- End of Dynamic Content Gallery jQuery gallery -->';
			$output .= "\n" . $errmsgs['11'];
...
function dfcg_jq_onecategory_method_gallery() {
	...
	$output .= "\n\t\t" . '<div id="dfcg-fullsize">';
	$output .= "\n\t\t\t" . '<div id="dfcg-imgprev" class="dfcg-imgnav" title="Previous Image"></div>';
	$output .= "\n\t\t\t" . '<a style="display:block;" id="dfcg-imglink">&nbsp;</a>';
	$output .= "\n\t\t\t" . '<div id="dfcg-imgnext" class="dfcg-imgnav" title="Next Image"></div>';
	$output .= "\n\t\t\t" . '<div id="dfcg-image"></div>';
	$output .= "\n\t\t\t" . '<div id="dfcg-text">';
	$output .= "\n\t\t\t\t" . '<h3></h3>';
...
function dfcg_jq_id_method_gallery() {
	...
	if( $ids_found ) {
		...
		foreach( $ids_found as $id_found ) :
			...
			// Output the images
			$output .= "\n\t\t\t" . '<img class="' . $image['class'] . ' full" src="'. $image['src'] .'" alt="'. $title .'" />';
			$output .= $errmsgs[$image['msg']];
