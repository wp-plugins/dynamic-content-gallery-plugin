 ...
 * Script version 1.0
 *
 * Loaded by dfcg_load_admin_scripts() which is hooked to admin_print_scripts-{$pagehook}
 *
 * @since 4.0
 */
...
jQuery(document).ready(function($) {
	...
	$(".dfcg-panel-help-link").click(function() { // bind click event to link
    	$tabs.tabs("select", 8); // switch to ninth tab
    	return false;
	});
});
...
// Cluetip
jQuery(document).ready(function($) {
	$('a.dfcg-local-tip').cluetip({local:true, cursor: 'pointer', sticky: true, closePosition: 'title', draggable: true, width: 400});
});